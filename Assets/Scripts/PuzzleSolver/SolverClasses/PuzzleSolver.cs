...
namespace Assets.Scripts.PuzzleSolvers.SolverClasses
{
    public abstract class PuzzleSolver : IComparer<(Node, (int, int))>
    {
        ...
        public SerializeblePuzzleSolver Implicit()
        {
            var solver = GetMyPuzzleSolver();
            solver.GoalState = mGoalState;
            solver.StartState = mStartNode.mPuzzle;
            solver.WillItBeContinued = mWillItBeContinued;
            return solver;
        }
        protected virtual SerializeblePuzzleSolver GetMyPuzzleSolver() => new SerializeblePuzzleSolver();
        public virtual PuzzleSolver Set(SerializeblePuzzleSolver solver)
        {
            mGoalState = solver.GoalState;
            mStartNode = new Node(solver.StartState);
            mSize = (sbyte)Math.Sqrt(mStartNode.mPuzzle.Length);
            mWillItBeContinued = solver.WillItBeContinued;
    ...
    [Serializable] public class SerializeblePuzzleSolver
    {
        ...
        public sbyte[] StartState;
        public bool WillItBeContinued;
    }
